...
def test_module_cli_stats_uses_configured_db_path(tmp_path):
    db_path = tmp_path / "clark.db"
    env = {**os.environ, "CLARK_DB_PATH": str(db_path)}

    result = subprocess.run(
        [sys.executable, "-m", "clark.memory", "stats"],
        check=True,
        capture_output=True,
        text=True,
        env=env,
...
def test_console_entrypoint_stats_uses_configured_db_path(tmp_path):
    db_path = tmp_path / "clark-console.db"
    env = {**os.environ, "CLARK_DB_PATH": str(db_path)}
...
def test_console_entrypoint_migrate_without_database_url_reports_sqlite_mode(tmp_path):
    env = {**os.environ, "CLARK_DB_PATH": str(tmp_path / "legacy.db")}
